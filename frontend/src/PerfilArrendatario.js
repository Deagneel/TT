...
const PerfilArrendatario = () => {
  ...
  const handleActualizarNombre = async () => {
    ...
    // Validar que tenga tres caracteres idénticos seguidos.
    if (/(\w)\1{2}/.test(formData.nombre)) {
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:3031/newName/${formData.id}`,
        {
  ...
  const handleActualizarApe = async () => {
    ...
    // Validar la longitud mínima.
    if (formData.primer_apellido.trim().length < 3) {
      swal("Error", "Ingresa un nombre válido.", "error");
      return;
    }

    // Validar que tenga tres caracteres idénticos seguidos.
    if (/(\w)\1{2}/.test(formData.primer_apellido)) {
      swal("Error", "Ingresa un nombre valido, por favor.", "error");
      return;
    }
    ...
    try {
      axios.defaults.withCredentials = true;
      const response = await axios.put(
        `http://localhost:3031/newApe/${formData.id}`,
        {
  ...
  const handleActualizarApe2 = async () => {
    ...
    // Validar la longitud mínima.
    if (formData.segundo_apellido.trim().length < 3) {
      swal("Error", "Ingresa un nombre válido.", "error");
      return;
    }

    // Validar que tenga tres caracteres idénticos seguidos.
    if (/(\w)\1{2}/.test(formData.segundo_apellido)) {
      swal("Error", "Ingresa un nombre valido, por favor.", "error");
      return;
    }
    ...
    try {
      axios.defaults.withCredentials = true;
      const response = await axios.put(
        `http://localhost:3031/newApe2/${formData.id}`,
        {
  ...
  const handleActualizarCorreo = async () => {
    ...
    try {
      axios.defaults.withCredentials = true;
      const response = await axios.put(
        `http://localhost:3031/newMail/${formData.id}`,
        {
  ...
  const handleActualizarINE = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:3031/newIne/${formData.id}`,
        {
  ...
  const handleCambiarContraseña = () => {
    axios
      .post("http://localhost:3031/recuperar-contrasena", {
        correo: formData.correo,
      })
      .then((response) => {
        console.log(response.data);
  ...
  const handleCredencialEstudiante = async () => {
    ...
    try {
      ...
      // Realiza la solicitud PUT para actualizar el INE
      const response = await axios.put(
        `http://localhost:3031/newCredencial/${formData.id}`,
        {
          correo: imageResponse.data.url,
        }
      );
      ...
      swal(
        "Credencial de Estudiante Actualizada Correctamente",
        " ",
        "success"
      );
  ...
  const handleComprobanteInscripcion = async () => {
    ...
    try {
      ...
      // Realiza la solicitud PUT para actualizar el INE
      const response = await axios.put(
        `http://localhost:3031/newComprobante/${formData.id}`,
        {
          correo: imageResponse.data.url,
        }
      );
      ...
      console.log(response.data); // Puedes manejar la respuesta exitosa aquí
      swal(
        "Comprobante de Inscripción Actualizado Correctamente",
        " ",
        "success"
      );
  ...
  const handleComprobanteDomicilio = async () => {
    ...
    try {
      ...
      // Realiza la solicitud PUT para actualizar el INE
      const response = await axios.put(
        `http://localhost:3031/newComprobanteD/${formData.id}`,
        {
          correo: imageResponse.data.url,
        }
      );
      ...
      console.log(response.data); // Puedes manejar la respuesta exitosa aquí
      swal(
        "Comprobante de Domicilio Actualizado Correctamente",
        " ",
        "success"
      );
  ...
  const handleEliminarIncidencia = async (repoId) => {
    try {
      const rentadosCheckResponse = await axios.get(
        `http://localhost:3031/verificarEstadoRentados`
      );
      ...
      if (tieneEstadoActivo == 0) {
        ...
        await swal(
          "No puedes eliminar el perfil porque hay al menos un inmueble rentado"
        );
      } else {
        ...
        <div className="col-md-8 col-lg-6">
          <div className="bg-white p-4 rounded text-center">
            ...
            <div className="mb-3 row align-items-center">
              ...
              <div className="col-sm-6">{formData.correo}</div>
            </div>
            ...
            <div className="mb-3 row align-items-center">
              ...
              <div className="col-sm-3">
                <button
                  className="btn btn-secondary"
                  onClick={handleCredencialEstudiante}
                >
                  Subir
                </button>
              </div>
            ...
            <div className="mb-3 row align-items-center">
              ...
              <div className="col-sm-3">
                <button
                  className="btn btn-secondary"
                  onClick={handleComprobanteInscripcion}
                >
                  Subir
                </button>
              </div>
            ...
            <div className="mb-3 row align-items-center">
              ...
              <div className="col-sm-3">
                <button
                  className="btn btn-secondary"
                  onClick={handleComprobanteDomicilio}
                >
                  Subir
                </button>
              </div>
            ...
            <div className="row">
              ...
              <div className="col-12">
                <button
                  className="btn btn-dark btn-block"
                  onClick={() => handleEliminarIncidencia(formData.id)}
                >
                  Eliminar cuenta
                </button>
              </div>
