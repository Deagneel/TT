    ...
    const obtenerEscuela = async () => {
      try {
        const response = await fetch(
          `http://localhost:3031/escuela/${idEscuela}`
        );
        if (!response.ok) {
        ...
        const { latitud, longitud, nombre, foto, ...restoDatosEscuela } =
          escuela;
        const nuevaPosition = {
  ...
  useEffect(() => {
    const obtenerInmuebles = async () => {
      if (position.lat !== 0 && position.lng !== 0 && nombreEscuela !== null) {
        ...
        try {
          const response = await fetch(
            "http://localhost:3031/infoinmueblesmap"
          );
          if (!response.ok) {
          ...
          const inmueblesFiltrados = data.filter((inmueble) => {
            const distanciaEscuelaHabitacion = getDistanceFromLatLonInKm(
              inmueble.latitud,
              inmueble.longitud,
              position.lat,
              position.lng
            ...
            console.log(
              "Número de habitaciones cumple criterio:",
              inmueble.no_habitaciones > 0
            );
            console.log("Precio cumple criterio:", inmueble.precio <= precio);
            console.log(
              "Tipo de habitación cumple criterio:",
              inmueble.tipo_de_habitacion == tipoHabitacion
            );
            console.log(
              "Distancia cumple criterio:",
              distanciaEscuelaHabitacion <= distancia
            );
              ...
              <div className="container" style={{ maxWidth: "300px" }}>
                ...
                <h6>
                  ...
                  {getDistanceFromLatLonInKm(
                    selectedInmueble.latitud,
                    selectedInmueble.longitud,
                    position.lat,
                    position.lng
                  ).toFixed(2)}{" "}
                  km
                ...
                <button
                  ...
                  onMouseOver={(e) =>
                    (e.target.style.backgroundColor = "#9b8c5a")
                  }
                  onMouseOut={(e) =>
                    (e.target.style.backgroundColor = "#beaf87")
                  }
                  onClick={(e) => {
                    e.preventDefault();
                    navigate(
                      `/infoinmueble?id_inmueble=${selectedInmueble.id_inmueble}`
                    );
                  }}
                >
                  Ver Detalles
        ...
        <div className="bg-secondary d-flex flex-wrap justify-content-around p-3">
          <div className="mb-3">
            ...
            <select
              ...
              onChange={(e) => handleSelectChange(e, "precio")}
            >
          ...
          <div className="mb-3">
            <label htmlFor="distancia" className="text-white">
              Distancia:
            </label>
            <select
              id="distancia"
              className="form-select"
              onChange={(e) => handleSelectChange(e, "distancia")}
            >
              {Array.from({ length: 30 }, (_, i) => i + 1).map((km) => (
                <option key={km} value={km}>
                  {km} km
                </option>
              ))}
              <option value="300">Más de 30 km</option>
            </select>
          </div>

          <div className="mb-3">
            <label htmlFor="tipoHabitacion" className="text-white">
              Tipo de Habitación:
            </label>
            <select
              id="tipoHabitacion"
              className="form-select"
              onChange={(e) => handleSelectChange(e, "tipoHabitacion")}
            >
